...
if (isset($_POST['precisase']) && $_POST['precisase'] != "" ) {
   ...
   $sql  = "INSERT INTO anuncios (codigo_usuario, precisase, descricao, telefone, email, site, endereco, data_insercao, data_vencimento) ";
   $sql .= "VALUES ($codigousuario, '$precisase', '$descricao', '$telefone', '$email', '$site', '$endereco', '$datainsercao', '$datavencimento');";
   $resultado = banco($sql);
    ...
    if (count($emails) > 0) {
        ...
        foreach ($emails as $emaill) {
            ...
            $mail->SetFrom(getenv("EMAIL"), "voluntárioJobs");
            ...
            $body .= "<p><a href= 'http://$_SERVER[HTTP_HOST]" . "model/cancelar_anuncio.php?email=" . $emaill . "'>Clique aqui para parar de receber e-mail sobre esta pesquisa</a></p>";
            $body .= "</body></html>";
